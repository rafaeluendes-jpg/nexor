  ...
  return new Promise(resolve => {
      ...
      beforeParse(w) {
        ...
        if (comSessaoGuardada) {
          ...
          try {
            ...
            w.localStorage.setItem('nexor_ultimo_email', 'santafe@jologelato.com.br');
    ...
    setTimeout(() => {
      ...
      resolve({
        ...
        emailPreenchido: (d.getElementById('lgU') || {}).value || '',
        sessaoGuardada: (function(){ try{ return w.localStorage.getItem('nexor_sessao'); }catch(e){ return 'erro'; } })(),
        formLoginCheio: cheio('loginForm'),
        engolidos: (function () {
          try {
...
(async function () {
  ...
  grupo('Abrindo COM sessão guardada — o login aparece SEMPRE (ordem do Rafael, 02/09/2026)');

  const b = await abrir(true);
  /* Antes o sistema entrava direto quando havia sessão guardada. O Rafael:
     "fechou a página e clica de novo, ele já entra direto; tem que seguir o
     processo — clicou, aparece o login, daí entra." Agora para no login. */
  t('a tela de login aparece, mesmo com sessão guardada', !b.loginEscondido, JSON.stringify({login:b.loginEscondido,app:b.appVisivel}));
  t('o sistema NÃO entra sozinho', !b.appVisivel);
  t('o formulário de login está desenhado (não é tela branca)', b.formLoginCheio);
  t('o e-mail volta preenchido, para agilizar', b.emailPreenchido === 'santafe@jologelato.com.br', b.emailPreenchido);
  t('a sessão guardada antiga foi descartada', b.sessaoGuardada === null, b.sessaoGuardada);
  ...
  grupo('O sistema NÃO entra sozinho');

  t('ninguém foi logado sem passar pelo login',
