...
async function entrarPeloAuth(lg,sn){
  ...
  if(_mesmoDono){
    DB=_ant;
    try{ semear(); }catch(eS3){_quieto(eS3,'entrarPeloAuth')}
...
async function entrar(){
  ...
  if(u){
    ...
    salvar();
    /* "Manter conectado" ligado: a sessão fica no aparelho e ele volta direto.
    ...
    try{
      ...
      if(manter){
        ...
        sessionStorage.setItem('nexor_sessao',SESSAO.login);
      }
...
try{
  ...
  try{
    var _mk=localStorage.getItem('nexor_manter');
    if(_mk==='0'){var _c=document.getElementById('lgK');if(_c)_c.checked=false;}
  }catch(e){_quieto(e,'sair')}
  if(_ses){
    try{ if(!DB||!DB.usuarios)carregar(); }catch(e){_quieto(e,'sair')}
    baseUsr();
    if(_ses==='1'){try{localStorage.removeItem('nexor_sessao')}catch(e){_quieto(e,'sair')}}
    else{
      var _k=String(_ses).toLowerCase();
      var _u=(DB.usuarios||[]).find(function(x){return String(x.login||'').toLowerCase()===_k})
          ||(DB.usuarios||[]).find(function(x){return x.id===_ses});
