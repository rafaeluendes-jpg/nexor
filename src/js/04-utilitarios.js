...
(function guardarRolagem(){
  /* captura na fase de captura porque a barra e de um elemento interno */
  document.addEventListener('scroll',function(e){
    if(_rolTrava)return;
    var el=e.target;
    if(el&&el.classList&&(el.classList.contains('etScroll')||el.classList.contains('finWrap')))
      _rolTela[chaveTela()]=el.scrollTop;
  },true);
  window.addEventListener('scroll',function(){
    if(_rolTrava||areaRolagem())return;
    _rolTela[chaveTela()]=window.scrollY||0;
  });
})();
function devolverRolagem(){
  ...
  _rolTrava=true;
  var el=areaRolagem();
  if(el){ if(Math.abs(el.scrollTop-y)>2)el.scrollTop=y; }
  else if(Math.abs((window.scrollY||0)-y)>2)window.scrollTo(0,y);
