...
function telaProducao(){
  ...
  /* totais por unidade: cascao e em unidades, gelato e em kg — somar os
     dois como "kg" era o erro da tela (Rafael, 17/09/2026) */
  var porU={};
  lista.forEach(function(o){
    var t=totaisPorUnidadeOP(o);
    Object.keys(t).forEach(function(u){porU[u]=porU[u]||{prev:0,real:0};porU[u].prev+=t[u].prev;porU[u].real+=t[u].real;});
  });
  ...
  $('content').innerHTML='<div class="etWrap"><div class="etScroll">'+
   ...
   '<div class="etTopo">'+
    ...
    '<div class="etTot">'+
     ...
     (Object.keys(porU).length?htmlTotaisOP(porU):'<div class="etT"><span>Previsto</span><b>—</b></div>')+
    '</div>'+
   ...
   (lista.length?'<table class="etTab semBusca"><thead><tr>'+
    ...
    lista.map(function(o){
      ...
      '<td>'+E(o.resp||'—')+'</td>'+
      '<td style="text-align:right">'+textoQtdOP(totaisPorUnidadeOP(o),'prev')+'</td>'+
      '<td style="text-align:right"><b>'+textoQtdOP(totaisPorUnidadeOP(o),'real')+'</b></td>'+
      '<td style="text-align:right"><b class="'+(dif>=0?'vg':'vr')+'">'+
       textoQtdOP(totaisPorUnidadeOP(o),'dif')+'</b></td>'+
...
/* a unidade de um item de ordem: gravada na ordem; ordem antiga, sem ela,
   pergunta a ficha (o destino manda: cascao em un, gelato em kg) */
function unidadeItemOP(it){
  if(it&&it.unidade)return it.unidade;
  var f=(DB.fichas||[]).find(function(x){return x.id===(it&&it.fichaId)});
  if(!f)return 'kg';
  try{ return previstoDestino(f).unidade||f.unidade||'kg'; }catch(e){ return f.unidade||'kg'; }
}
/* previsto/produzido de uma ordem gravada, separados por unidade */
function totaisPorUnidadeOP(o){
  var por={};
  ((o&&o.itens)||[]).forEach(function(it){
    var u=unidadeItemOP(it)||'kg';
    por[u]=por[u]||{prev:0,real:0};
    por[u].prev+=Number(it.previsto)||0;
    por[u].real+=Number(it.real)||0;
  });
  if(!Object.keys(por).length&&o)por.kg={prev:Number(o.previsto)||0,real:Number(o.real)||0};
  return por;
}
/* "300 un" · "55,07 kg · 40 un" — campo: prev, real ou dif */
function textoQtdOP(por,campo){
  var ks=Object.keys(por||{});
  if(!ks.length)return '—';
  return ks.map(function(u){
    var v=campo==='dif'?(por[u].real-por[u].prev):por[u][campo];
    return (campo==='dif'&&v>0?'+':'')+fmtQt(v)+' '+un(u).ab;
  }).join(' · ');
}
  ...
  var op={id:uid('op'),numero:proxNumOP(),data:OP.data||hojeISO(),hora:agoraHM(),
    ...
    itens:OP.itens.map(function(it){
      ...
      return {fichaId:it.fichaId,nome:it.nome,receitas:recOP(it),previsto:Number(it.previsto)||0,
        unidade:it.unidade||'kg',destinoNome:it.destinoNome||'',
...
function verOP(id){
  ...
  '<div class="acKpis">'+
   ...
   '<div class="acK"><span>Previsto</span><b>'+textoQtdOP(totaisPorUnidadeOP(o),'prev')+'</b></div>'+
   '<div class="acK"><span>Produzido</span><b>'+textoQtdOP(totaisPorUnidadeOP(o),'real')+'</b></div>'+
   '<div class="acK dest3"><span>Diferença</span><b class="'+((o.diferenca||0)>=0?'vg':'vr')+'">'+
    textoQtdOP(totaisPorUnidadeOP(o),'dif')+'</b></div>'+
  '</div>'+
  '<div class="blk" style="margin:0;max-width:none;padding:0;overflow:hidden">'+
   ...
   '<div class="acTabW" style="max-height:340px"><table class="acTab"><thead><tr>'+
    ...
    (o.itens||[]).map(function(it){
      ...
      '<td style="text-align:right">'+fmtQt(it.previsto)+' '+un(unidadeItemOP(it)).ab+'</td>'+
      '<td style="text-align:right;color:var(--ink-3)">'+
       (it.cubas||[]).filter(function(c){return c}).map(function(c){return fmtQt(c)}).join(' + ')+'</td>'+
      '<td style="text-align:right"><b>'+fmtQt(it.real)+' '+un(unidadeItemOP(it)).ab+'</b></td>'+
      '<td style="text-align:right"><b class="'+((it.diferenca||0)>=0?'vg':'vr')+'">'+
       ((it.diferenca||0)>0?'+':'')+fmtQt(it.diferenca)+' '+un(unidadeItemOP(it)).ab+'</b></td></tr>';
...
async function excluirOP(id){
  ...
  var ok=await confirmar({
    ...
    linhas:[['Produzido',textoQtdOP(totaisPorUnidadeOP(o),'real'),''],
...
function imprimirOP(id){
  ...
  '<table style="width:100%;font-size:11px;border-collapse:collapse">'+
   ...
   (o.itens||[]).map(function(it){
     ...
     '<td align="right">'+fmtQt(it.previsto)+' '+un(unidadeItemOP(it)).ab+'</td>'+
     '<td align="right">'+(it.cubas||[]).filter(function(c){return c}).map(function(c){return fmtQt(c)}).join(' + ')+'</td>'+
     '<td align="right">'+fmtQt(it.real)+' '+un(unidadeItemOP(it)).ab+'</td>'+
     '<td align="right">'+((it.diferenca||0)>0?'+':'')+fmtQt(it.diferenca)+'</td></tr>';
   }).join('')+
   '<tr style="border-top:1px solid #000"><td><b>TOTAL</b></td>'+
   '<td align="right"><b>'+textoQtdOP(totaisPorUnidadeOP(o),'prev')+'</b></td><td></td>'+
   '<td align="right"><b>'+textoQtdOP(totaisPorUnidadeOP(o),'real')+'</b></td>'+
