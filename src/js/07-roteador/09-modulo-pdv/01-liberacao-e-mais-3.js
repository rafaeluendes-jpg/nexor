...
/* ==========================================================
   ...
   03/09/2026. Regressão: os lançamentos de venda sumiram do Financeiro.
   Causa raiz — a semente de fábrica repunha, POR CIMA da configuração real
   da loja, formas com taxa de fábrica (1,99 / 3,49) e SEM conta de destino
   (débito, crédito e Pix com conta vazia). O fechamento cria o lançamento
   na conta da forma; conta vazia = lançamento sem conta = some do extrato
   do Banco Itaú. O mesmo caminho já tinha derrubado a taxa antes (V290).

   V290 travou a semente, mas cegou o caixa: sem forma nenhuma, não dava
   para finalizar. V293 reabriu a semente e trouxe a regressão de volta.

   A saída que atende os dois: a lista GRAVADA (`DB.formasPag`, que sobe
   para a nuvem) só recebe fábrica quando a nuvem NUNCA soube das formas
   (loja nova, sem uuid) — nunca por cima do que já existe. E o caixa nunca
...
function syncFormas(){
  /* a lista REAL da loja manda. Se ela ainda não chegou (vazia, esperando
     ...
     sem forma — essa lista não é gravada nem sobe para a nuvem, então não
     repõe taxa nem conta por cima do que a loja configurou. */
  var _base=(DB.formasPag&&DB.formasPag.length)?DB.formasPag:_formasFabrica();
  FORMAS=_base.filter(function(f){return f.ativa!==false})
