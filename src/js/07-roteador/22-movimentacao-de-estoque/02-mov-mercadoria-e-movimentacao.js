...
function telaMovimentacao(){
  ...
  /* ==========================================================
     MEDIA E SOMA DIVIDIDA PELO NUMERO DE LANCAMENTOS
     Tres entradas de 10 kg somam 30 e a media e 10 — nao 30. O rodape
     mostrava a soma com o nome de media, que e pior que nao mostrar:
     o numero parece conferido e nao e o que diz ser.
     ========================================================== */
  ...
  var qtdEnt=ent.length?somaEnt/ent.length:0;   /* media por lancamento */
  var qtdSai=sai.length?somaSai/sai.length:0;
  /* ==========================================================
  ...
  $('content').innerHTML='<div class="mvWrap">'+
   ...
   '<div class="mvFiltros">'+
    ...
    '<div class="f2" style="min-width:210px"><label>Item</label>'+
     '<input id="mvBusca" list="mvLista" placeholder="digite 3 letras ou o código" autocomplete="off" '+
     ...
     itensEstoque().slice().sort(function(a,b){return (a.nome||'').localeCompare(b.nome||'')})
       .map(function(i){return '<option value="'+E(i.nome)+'">'+E(i.codigo||'')+' · '+
   ...
   (linhas.length?'<table class="mvTab"><thead><tr>'+
    ...
    '<tfoot><tr>'+
     ...
     '<td style="text-align:right"><b>'+fmtQt(qtdEnt)+(unMedia?' '+E(unMedia):'')+'</b>'+
      '<small>média de '+ent.length+' entrada'+(ent.length===1?'':'s')+'</small></td>'+
     '<td style="text-align:right"><b>'+fmtQt(qtdSai)+(unMedia?' '+E(unMedia):'')+'</b>'+
      '<small>média de '+sai.length+' consumo'+(sai.length===1?'':'s')+'</small></td>'+
  ...
  if(mb){
    ...
    mb.oninput=function(){ _focoBusca='mvBusca'; _aplicaBuscaMov(this.value,false); };
    mb.onchange=function(){ _focoBusca='mvBusca'; _aplicaBuscaMov(this.value,true); };
    mb.onblur=function(){ var v=this.value; setTimeout(function(){_aplicaBuscaMov(v,true)},180); };
    mb.onkeydown=function(e){ if(e.key==='Enter'){ _focoBusca='mvBusca'; _aplicaBuscaMov(this.value,true); } };
  }
