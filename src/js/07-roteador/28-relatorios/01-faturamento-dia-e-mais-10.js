...
function cfgDRE(){
  ...
  /* CPV e o que foi CONSUMIDO pelas vendas — nunca nota de entrada. Nota e
     compra de estoque, nao custo do que se vendeu. A chave fica so por
     compatibilidade e e sempre desligada (12/09/2026, ordem do Rafael). */
  c.cpvPorCompra=false;
  return c;
}
...
/* ---------- cálculo ---------- */
/* ==========================================================
   O DRE ABRE EM CASCATA (12/09/2026)

   O Rafael: "dou o mais no Faturamento Total, ele abre Vendas Diretas;
   mais um, abre quanto foi de gelato, de cascao... O CPV abre por grupo
   ate chegar no produto. Royalties abre por unidade. Pessoal abre
   salario, e dentro o nome da pessoa."

   Antes o calculo devolvia so o total de cada rubrica por mes. Agora,
   alem do total, guarda DE ONDE veio cada centavo, numa arvore por
   rubrica (m.det): venda -> grupo do cardapio -> produto; CPV -> grupo
   -> produto -> ingrediente; lancamento -> categoria -> subcategoria ->
   unidade -> lancamento; DFV -> forma de pagamento. A tela so abre e
   fecha os galhos: a conta e uma so, e a soma dos galhos E o total da
   rubrica — nunca dois numeros para a mesma coisa.

   CPV e o que foi CONSUMIDO pelas vendas, ao custo medio da baixa. Nota
   de entrada e compra de estoque, nao custo do que se vendeu.
   ========================================================== */
function calcularDRE(ano){
  ...
  function noDe(pai,rot){
    pai.ch=pai.ch||{};
    if(!pai.ch[rot])pai.ch[rot]={n:rot,v:zero(),ch:null};
    return pai.ch[rot];
  }
  function add(rub,caminho,k,val){
    val=Number(val)||0; if(!val)return;
    det[rub]=det[rub]||{n:rub,v:zero(),ch:null};
  ...
  /* produto dono de cada ficha: e por ele que o CPV chega ao grupo do cardapio */
  var prodPorFicha={};(DB.produtos||[]).forEach(function(p){if(p.fichaId&&!prodPorFicha[p.fichaId])prodPorFicha[p.fichaId]=p});
  var fichaPorNome={};(DB.fichas||[]).forEach(function(f){var n=String(f.nome||'').toLowerCase().trim();if(n&&!fichaPorNome[n])fichaPorNome[n]=f});
  var multiUn=false; try{multiUn=(typeof sucAtivas==='function'&&sucAtivas().length>1)}catch(e){multiUn=false}
  var unDe=function(x){var s=(x&&(x.sucursalId||x._suc))||'';if(!s)return 'Sem unidade';
  ...
  (DB.pedidos||[]).forEach(function(p){
    ...
    (p.itens||[]).forEach(function(it){
      var v=Number(it.total)||0; soma+=v;
      var pr=prodPorId[it.produtoId];
      add('01.01',[pr?catNome(pr.categoriaId):'Sem grupo', it.nome||(pr&&pr.nome)||'Item'],k,v);
    });
    var resto=+(tot-soma).toFixed(2);
    if(Math.abs(resto)>=0.005)add('01.01',['Taxas, acréscimos e descontos', resto>0?'Taxa de entrega / acréscimo':'Desconto'],k,resto);
    m[k]['01.01']+=tot;
  });
  ...
  (DB.movEst||[]).forEach(function(mv){
    ...
    (mv.linhas||[]).forEach(function(l){
      ...
      var fnome=l.fichaNome||'';
      var f=fnome?fichaPorNome[fnome.toLowerCase().trim()]:null;
      var pr=f?prodPorFicha[f.id]:null;
      var grupo=pr?catNome(pr.categoriaId):(f?((typeof nomeSubProd==='function')?nomeSubProd(f):'Produzido'):'Sem grupo');
      var produto=fnome||l.nome||'Item';
      var cam=[grupo,produto]; if(fnome)cam.push(l.nome||'Ingrediente');
      m[k]['02']+=v; add('02',cam,k,v);
    });
  });
  ...
  (DB.lancFin||[]).forEach(function(l){
    ...
    var partes=String(nomeCat||'').split(' › ');
    var cam=[(partes[0]&&partes[0]!=='—')?partes[0]:(l.categoriaTxt||'Sem categoria')];
    if(partes[1])cam.push(partes[1]);
    if(multiUn)cam.push(unDe(l));
    cam.push((l.descricao||'Lançamento')+(l.fornecedor?' · '+l.fornecedor:'')+(dt?' · '+dataBR(dt):''));
    add(rub,cam,k,v);
  });
  /* percentuais automaticos sobre o faturamento */
...
/* ---------- TELA DO DRE ---------- */
/* rubricas-pai e seus filhos: o "+" do pai mostra ou esconde os filhos */
var FILHOS_RUB={'01':['01.01','01.02'],'04':['04.01','04.02'],'11':['12','13'],'14':['15','16','17']};
var PAI_RUB={};Object.keys(FILHOS_RUB).forEach(function(p){FILHOS_RUB[p].forEach(function(f){PAI_RUB[f]=p})});
var DRE={ano:0,tri:0,mes:0,ab:null};
function telaDRE(){
  baseMov();baseFormas();
  ...
  /* um mes so: nome, valor e percentual ficam lado a lado, e a sobra da
     largura vai para uma coluna vazia no fim — antes o valor ia parar no
     meio da tela e o percentual na outra ponta */
  var compacta=cols.length===1;
  ...
  $('content').innerHTML='<div class="etWrap"><div class="etScroll">'+
   ...
   '</div>'+
   '<div class="etTabW plano2" id="relArea">'+
   '<table class="etTab dreTab'+(compacta?' compacta':'')+'"><thead><tr>'+
    '<th style="width:56px">Rubrica</th><th style="width:'+(compacta?320:230)+'px">Elemento</th>'+
    ...
    (compacta?'<th class="dreEnche"></th>':'')+
   '</tr></thead><tbody id="dreCorpo">'+corpoDRE(m,cols)+'</tbody></table></div></div></div>';
  DRE._m=m;DRE._cols=cols;
  rodape('DRE '+(DRE.mes?MESES_DRE[DRE.mes-1]+'/':'')+DRE.ano+' · regime de '+
    (cfgDRE().regime==='caixa'?'caixa':'competência'));
}
/* so as linhas da tabela. Separado da tela para o "+"/"−" trocar SO o corpo:
   antes, cada clique recalculava o ano inteiro e remontava a tela toda
   (cabecalho, filtros, tabela) — a tela piscava e pulava a cada abrir/fechar. */
function corpoDRE(m,cols){
     function pc(v,fat){ if(!fat)return '—'; return (v/fat*100).toFixed(1).replace('.',',')+'%'; }
     var det=m.det||{}, out='', compacta=cols.length===1;
     function celulas(getV,cem){
       var totAno=0,fatAno=0;
       cols.forEach(function(k){totAno+=getV(k)||0;fatAno+=m[k]['01']||0;});
       return cols.map(function(k){
         var v=getV(k)||0, fat=m[k]['01']||0;
         return '<td class="dreV'+(v<0?' neg':'')+'">'+(v?money(v):'—')+'</td>'+
                '<td class="drePc">'+(cem?'100%':pc(v,fat))+'</td>';
       }).join('')+
       (cols.length>1?
        '<td class="dreV colAno'+(totAno<0?' neg':'')+'"><b>'+(totAno?money(totAno):'—')+'</b></td>'+
        '<td class="drePc colAno">'+(cem?'100%':pc(totAno,fatAno))+'</td>':'')+
       (compacta?'<td class="dreEnche"></td>':'');
     }
     function temFilhos(no){return !!(no&&no.ch&&Object.keys(no.ch).length)}
     /* o galho mais pesado primeiro: e o que a pessoa quer ver */
     function filhos(no){
       return Object.keys(no.ch).map(function(x){return no.ch[x]}).sort(function(a,b){
         var sa=0,sb=0; cols.forEach(function(k){sa+=a.v[k]||0;sb+=b.v[k]||0}); return Math.abs(sb)-Math.abs(sa);});
     }
     function botao(chave,aberto){
       return '<button class="dreTg" data-k="'+E(chave)+'" onclick="toggleDre(this)" '+
        'title="'+(aberto?'Recolher':'Abrir o nível de baixo')+'">'+(aberto?'−':'+')+'</button>';
     }
     function galho(no,chave,nivel){
       var aberto=!!DRE.ab[chave], tem=temFilhos(no);
       out+='<tr class="dreDet n'+nivel+'"><td class="dreC"></td>'+
        '<td class="dreN" style="padding-left:'+(24+nivel*16)+'px" title="'+E(no.n)+'">'+
        (tem?botao(chave,aberto):'<span class="dreTg vazio"></span>')+E(no.n)+'</td>'+
        celulas(function(k){return no.v[k]},false)+'</tr>';
       if(aberto&&tem)filhos(no).forEach(function(f){galho(f,chave+'|'+f.n,nivel+1)});
     }
     RUBRICAS.forEach(function(r){
       if(PAI_RUB[r.c]&&!DRE.ab[PAI_RUB[r.c]])return;      /* pai fechado esconde os filhos */
       var cls=r.t==='sub'?'dreSub':(r.t==='res'?'dreRes':(r.t==='fin'?'dreFin':
         (r.t==='tot'?'dreTot':'dreLin')));
       var d=det[r.c], temDet=temFilhos(d), ehPai=!!FILHOS_RUB[r.c];
       var aberto=!!DRE.ab[r.c];
       out+='<tr class="'+cls+'">'+
...
/* abre/fecha um galho do DRE sem remontar a tela: troca so as linhas da
   tabela, com os numeros ja calculados. Nada pisca, a rolagem nao se mexe. */
function toggleDre(btn){
  var k=btn&&btn.getAttribute?btn.getAttribute('data-k'):String(btn||'');
  if(!k)return;
  DRE.ab=DRE.ab||{}; DRE.ab[k]=!DRE.ab[k];
  var tb=$('dreCorpo');
  if(tb&&DRE._m&&DRE._cols){ tb.innerHTML=corpoDRE(DRE._m,DRE._cols); return; }
  var sc=document.querySelector('.etScroll'); var y=sc?sc.scrollTop:0;
  telaDRE();
  var sc2=document.querySelector('.etScroll'); if(sc2)sc2.scrollTop=y;
}
...
function explicaDRE(){
  ...
  explicaRel('DRE — como é feito',[
   ...
   ['02 CPV','custo das mercadorias que saíram por venda, pelo custo médio no momento da baixa '+
     '— nota de entrada é compra de estoque e não entra aqui'],
   ['O botão +','cada rubrica abre em cascata: venda → grupo → produto; CPV → grupo → produto → ingrediente; '+
     'despesa → categoria → subcategoria → unidade → lançamento; DFV → forma de pagamento'],
     ...
     '<div class="colB">'+
      ...
      RUBRICAS.filter(function(r){return r.t!=='sub'||r.c==='01.01'}).map(function(r){
        ...
        var auto=(r.c==='01.01'?'vendas do PDV':
          r.c==='02'?'custo do que foi vendido':
