...
/* ==========================================================
   O AVISO ERA "UMA VEZ", MAS SO SE CLICASSEM — E NINGUEM CLICAVA (05/09/2026)

   A intencao sempre foi mostrar este aviso UMA VEZ por aparelho. Mas o
   `nexor_impressao_ok` so era gravado quando a pessoa clicava "Ja
   ajustei" — e Santa Fe do Sul nunca clicou. Resultado: o aviso reaparecia
   ANTES de CADA impressao, empilhando um modal na frente do papel. Ao
   fechar o caixa e mandar imprimir, o operador via esta janela em vez do
   comprovante — "clico imprimir e nao imprime; repito e ai sai".

   Agora, ao MOSTRAR, ja se lembra: aparece uma vez, e da impressao
   seguinte em diante a janela de impressao abre limpa, sozinha. O aviso e
   so uma dica cosmetica (o cabecalho/rodape do Chrome); imprimir de
   verdade nunca pode depender dele.
   ========================================================== */
function avisoJanelaImpressao(){
  try{
    if(localStorage.getItem('nexor_impressao_ok'))return;
    try{ localStorage.setItem('nexor_impressao_ok','1'); }catch(e){}   /* uma vez, de verdade */
    var o=document.createElement('div');o.className='mdOv';o.id='mdImpAviso';
    o.innerHTML='<div class="mdBox"><div class="mdH"><b>Ajuste a janela de impressão (uma vez)</b>'+
      '<button onclick="fecharAvisoImpressao(true)">&times;</button></div>'+
      ...
      'navegador. Na janela de impressão, desmarque uma vez:</div>'+
      htmlPassosImpressao()+
      '<div class="hint">O Chrome guarda essa escolha. Este aviso não aparece de novo.</div>'+
      '</div><div class="mdF">'+
      '<button class="btnP2 ok" onclick="fecharAvisoImpressao(true)">Entendi</button>'+
      '</div></div>';
