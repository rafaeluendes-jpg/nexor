...
function baseImp(){
  ...
  IMP_TIPOS.forEach(function(t,i){
    if(!DB.modelosImp.some(function(m){return m.tipo===t.id}))
      DB.modelosImp.push({id:uid('mi'),tipo:t.id,nome:t.n,colunas:48,vias:t.id==='ficha'?2:1,
...
function linhasItens(ped,cols,comPreco){
  ...
  (ped.itens||[]).forEach(function(i){
    ...
    var cab=q+'  '+nome.slice(0,Math.max(4,largNome));
    if(val){
      var esp=cols-cab.length-val.length;
      cab=cab+(esp>0?new Array(esp+1).join(' '):' ')+val;
    }
    r.push({tipo:'txt',txt:cab,al:'e'});
...
function linhasPag(ped,cols){
  ...
  if(!r.length)r.push({tipo:'txt',txt:'(nao informado)',al:'e'});
  return r;
}
...
function papelHTML(linhas,cols){
  return linhas.map(function(l){
    ...
    if(l.tipo==='corte')return '<div class="ppCorte">'+new Array(cols+1).join('- ')+'</div>';
    if(l.tipo==='barras')
...
function pedidoExemplo(tipo){
  return {numero:1042,senha:18,data:hojeISO(),hora:agoraHM(),
    ...
    pagamentos:[{formaId:(DB.formasPag||[{}])[0].id,valor:tipo==='entrega'?94:91}]};
}
...
function telaModeloImp(){
  ...
  $('content').innerHTML='<div class="etWrap"><div class="etScroll">'+
   ...
   '<div class="impGrade">'+
    '<div class="impEdit">'+
     '<div class="impBarra">'+
      '<div class="fl"><label>Largura do papel</label><select id="impCols" onchange="salvarImpCampo()">'+
       '<option value="32"'+(cols===32?' selected':'')+'>58 mm (bobina estreita)</option>'+
       '<option value="48"'+(cols===48?' selected':'')+'>80 mm (bobina larga)</option>'+
      '</select></div>'+
    ...
    '<div class="impPrev">'+
     ...
     '<div class="papel'+(cols===32?' p58':'')+'" id="impPapel">'+papelHTML(linhas,cols)+'</div>'+
    '</div>'+
   '</div>'+
   '</div></div>';
...
function previaImp(){
  ...
  el.className='papel'+(cols===32?' p58':'');
...
function salvarImpCampo(){
  var m=modeloImp(IMP.tipo);
  m.colunas=parseInt($('impCols').value,10)||48;
