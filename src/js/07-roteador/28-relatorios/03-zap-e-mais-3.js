...
async function aceitarPedidoOnline(id){
  ...
  if(!cli){
    ...
    if(end.bairro)cli.bairro=end.bairro;
  ...
  var ped={id:uid('ped'),numero:(DB.pedidos.length+1),
    ...
    /* ==========================================================
       O BAIRRO ESCRITO PELA PESSOA, E NAO A REGIAO DA TAXA

       A lista de regioes do cardapio existe para calcular a taxa e nao
       tem todos os bairros da cidade. Quem morava fora dela escolhia
       "Todos os Bairros", e era isso que saia impresso — o entregador
       recebia a rua sem saber em que bairro procurar.

       O cardapio passou a pedir o bairro escrito. Ele vai no pedido
       (para o papel sair completo agora) e na ficha do cliente (que
       sobe para a nuvem, para a segunda via em outro aparelho tambem
       sair completa).
       ========================================================== */
    bairro:end.bairro||'',
...
/* ==========================================================
   ...
   ========================================================== */
/* ==========================================================
   O BAIRRO DO PAPEL DA ENTREGA

   Sao duas coisas com o mesmo nome, e o entregador precisa da primeira:
     - o BAIRRO que a pessoa escreveu ("Jardim Alvorada");
     - a REGIAO da taxa, escolhida numa lista curta ("Todos os Bairros").

   Ate aqui so a segunda existia, e era ela que ia para o papel. Em
   30/08/2026 saiu "Todos os Bairros" e mais nada — o entregador teve de
   escrever o endereco a mao no proprio cupom.

   Agora sai o bairro escrito e, do lado, a regiao entre parenteses
   quando as duas nao sao a mesma coisa: a regiao e o que explica o
   valor da taxa, e some quando nao acrescenta nada.

   Pedido antigo, sem bairro escrito, continua saindo com a regiao — que
   e exatamente o que sempre saiu.
   ========================================================== */
function bairroDoPedido(ped){
  if(!ped)return '';
  var cli=(typeof _cliDoPedido==='function')?(_cliDoPedido(ped)||{}):{};
  var escrito=String(ped.bairro||cli.bairro||'').trim();
  var regiao=String(ped.zona||cli.zona||'').trim();
  if(!escrito)return regiao||String(ped.cidade||cli.cidade||'').trim();
  if(!regiao||regiao.toLowerCase()===escrito.toLowerCase())return escrito;
  return escrito+' ('+regiao+')';
}
...
/* ==========================================================
   ...
   Em 29/08/2026 o teste da Assistente devolveu isto na tela da loja:
   ...
   Ninguem numa sorveteria sabe o que fazer com isso — e a tela do Joia
   nao mostra texto tecnico. Pior: a frase nao diz o principal, que e
   "isto nao e defeito do sistema, e a conta da Meta que esta barrada".

   Esta funcao traduz as recusas conhecidas. O que ela nao conhecer sai
   sem o JSON, so com a frase, para nunca despejar chave nem rastro da
   Meta na frente do cliente.
   ========================================================== */
function motivoEnvioZap(bruto){
  var t=String(bruto||'').trim();
  if(!t)return 'o robô não respondeu.';
  /* a Meta barrou o aplicativo inteiro — nada sai enquanto isso durar */
  if(/API access blocked/i.test(t)||/"code"\s*:\s*200/.test(t))
    return 'o WhatsApp oficial (Meta) bloqueou o acesso do aplicativo. '+
      'Não é defeito do Joia: precisa ser liberado na conta da Meta.';
  /* senha de acesso vencida ou trocada */
  if(/"code"\s*:\s*190/.test(t)||/access token/i.test(t))
    return 'a senha de acesso da Meta venceu. Precisa gerar outra e '+
      'colocar no robô.';
  /* número fora da lista de teste, enquanto o aplicativo é de testes */
  if(/131030/.test(t)||/not in allowed list/i.test(t))
    return 'esse número ainda não está liberado na Meta para receber.';
  /* a janela de 24 horas */
  if(/131047/.test(t)||/24 hours/i.test(t)||/re-engagement/i.test(t))
    return 'a Meta só deixa escrever para quem falou com a Assistente nas '+
      'últimas 24 horas. Peça para o gerente mandar um "oi" para ela.';
  if(/não tem WhatsApp/i.test(t))return t;
  if(/não está conectada/i.test(t))
    return 'a loja não está conectada ao WhatsApp — leia o QR de novo.';
  if(/nenhum canal/i.test(t))
    return 'não há caminho de envio: nem a Meta nem o WhatsApp da loja '+
      'estão prontos.';
  if(/muitos envios/i.test(t))return t;
  /* desconhecido: tira o JSON e devolve so a frase */
  var limpo=t.replace(/\{[\s\S]*\}/,'').replace(/\(\d{3}\)\s*:?/,'').trim();
  return limpo||'o robô recusou o envio.';
}
/* ---------- envio ---------- */
async function avisarGerente(suc,tipo,texto){
  ...
  try{
    ...
    else if(r&&r.erro)toast('Não consegui avisar o gerente: '+motivoEnvioZap(r.erro));
  }catch(e){
    /* engolir o erro fazia parecer que tinha dado certo */
    toast('Não consegui avisar o gerente: '+motivoEnvioZap((e&&e.message)||''));
  }
}
function agoraBR(){
  var d=new Date();
...
function linhasFechamento(cx){
  baseFormas();
  /* ==========================================================
     O FECHAMENTO FICOU SOZINHO NA LETRA PEQUENA

     Quando a abertura, a sangria, o suprimento e o cancelamento
     ganharam comprovante, todos foram montados em 32 colunas na bobina
     de 80 mm — que e a letra que a loja aprovou. O fechamento ficou nas
     48 colunas de antes: a mesma bobina dividida em metade a mais de
     caracteres da 1,6 mm de largura de letra, contra 2,4. Legivel na
     tela, pequena demais no papel termico, e ele e justamente o papel
     que se le no fim da noite com pouca luz.

     Agora usa a MESMA conta das outras: 32 na bobina larga, 24 na
     estreita. O desenho ja e todo calculado a partir de `cols` — os
     cabecalhos encurtam sozinhos para "Qt" e "Valor" abaixo de 40
     colunas, e a largura do nome da forma sai da subtracao. Nada
     precisou ser reescrito a mao.
     ========================================================== */
  var papel=80;
