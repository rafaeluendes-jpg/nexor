...
function modalLanc(id,tipoNovo,pre){
  ...
  /* ==========================================================
     FORMA DE PAGAMENTO NO "JA ESTA PAGO" (05/09/2026)

     Banco e forma pertencem ao PAGAMENTO, nao ao lancamento — por isso so
     apareciam na hora de dar baixa (o joinha). Mas quando a conta ja
     nasce PAGA, quem lanca ja sabe como pagou: precisa dizer ali, no
     mesmo lugar, sem depender de um segundo modal. Entao Banco + Forma +
     Data foram para dentro do bloco que abre com "Ja esta pago" (abaixo),
     reusando os MESMOS campos que a baixa grava (`lnC`/`lnM` -> `contaId`/
     `metodoId`). O aviso aqui em cima so lembra o caminho da baixa quando
     a conta ainda nao esta paga. */
  '<div class="avisoPg" id="avPgBaixa" style="'+(mostraPg?'display:none':'')+'">'+sv('help',12)+
   ' <span>Banco e forma de pagamento são escolhidos na hora de dar baixa, no <b>joinha</b> da '+
   'lista de lançamentos — ou marque <b>Já está pago</b> abaixo para informar agora.</span></div>'+
  ...
  '<span>Já está pago<small style="display:block;color:var(--ink-3)">marque para informar como e quando foi pago</small></span></label>'+
  '<div id="boxPg" style="'+(l&&l.pago?'':'display:none')+'">'+
   '<div class="row2" style="margin-top:12px">'+
    ...
    '</select></div>'+
    '<div class="fld2"><label>Forma de pagamento *</label><select id="lnM">'+
     '<option value="">Selecione uma opção</option>'+
    ...
    '</select></div>'+
   '</div>'+
   '<div class="fld2" style="margin-top:12px"><label>Data do pagamento</label>'+
  ...
  modal((l?'Editar lançamento':(P.titulo||(tipo==='receita'?'Nova receita':'Nova despesa'))),h,(P.botao||'Salvar'),function(){
    ...
    /* banco e forma só são exigidos quando a conta já está paga: um
       lançamento PENDENTE não escolhe isso (é na baixa). Os campos existem
       sempre no DOM agora (escondidos), então a exigência checa o "já pago". */
    ...
    var o={tipo:tp,
      /* banco e forma só entram quando está pago (senão são da baixa) */
      contaId:(jaPago&&$('lnC')?$('lnC').value:(l?(l.contaId||''):(P.contaId||''))),
      metodoId:(jaPago&&$('lnM')?$('lnM').value:(l?(l.metodoId||''):(P.metodoId||''))),descricao:d,valor:v,
    ...
    if(parc){
      ...
      /* uma parcela so (nota de entrada): e um lancamento unico, com o
         vencimento escolhido embaixo — sem "(1/1)" no nome */
      if(ds.length===1){
  ...
  $('lnP').onchange=function(){
    var pg=this.checked;
    $('boxPg').style.display=pg?'':'none';
    var a=$('avPgBaixa'); if(a)a.style.display=pg?'none':'';
  };
  if(P.categoriaId){
...
function previewParc(){
  ...
  var rot=$('lnPriRot'); if(rot)rot.textContent=(ds.length===1?'Vencimento':'1º vencimento');
  box.innerHTML='<div class="prevTit">'+(ds.length===1?'Vencimento único':'Parcelas que serão lançadas <b>'+ds.length+'</b>')+' · total R$ '+money(soma)+
