...
function volta(linhas,fn,atual,col){
  ...
  /* ==========================================================
     A CATEGORIA DO LANCAMENTO FICAVA NO APARELHO E CHEGAVA VAZIA NA NUVEM
     (24/09/2026 — dos 74 lancamentos "sem categoria" da auditoria RDS)

     O envio traduz a subcategoria do lancamento por fkSub(), que procura
     em _ids. `guardarIds` so registra a linha PAI (a categoria); as
     subcategorias, que descem aninhadas, nunca entravam no mapa. Num
     aparelho que recebeu o plano de contas pelo download — o de toda
     loja — a traducao falhava, `subcategoria_id` subia nulo, e o
     lancamento que a loja classificou aparecia sem categoria para a nuvem,
     a API e o DRE da rede. Na tela da loja ele continuava classificado.

     Agora cada subcategoria que desce entra no mapa pelo id que a tela usa
     (e pelo ref_local cru, que lancamentos antigos podem guardar). */
  try{
    DB._uuid=DB._uuid||{}; DB._uuid.catfinSub=DB._uuid.catfinSub||{};
    (cf||[]).forEach(function(c3){
      var pr3=c3.ref_local||c3.id;
      (c3.subcategorias_financeiras||[]).forEach(function(s3){
        if(!s3||!s3.id)return;
        [idFilho(pr3,s3.ref_local,s3.id), s3.ref_local].forEach(function(r3){
          if(r3){ DB._uuid.catfinSub[r3]=s3.id; _ids[r3]=s3.id; }
        });
      });
    });
  }catch(e){ _quieto(e,'idsSubcategorias'); }
  var fo=await _p03;
