...
/* ==========================================================
   ...
   ========================================================== */
/* ==========================================================
   A COPIA DA SESSAO — A TRAVA QUE FALTAVA

   29/08/2026, 9h. Santa Fe do Sul abriu o dia sem conseguir conectar. No
   servidor, uma unica recusa em /auth/v1/token e mais nada: nenhum
   pedido da loja a manha inteira. O sistema tentava religar a cada 8
   segundos, `getSession()` devolvia vazio, e ele desistia calado —
   mostrando "servidor nao respondeu", que nao era verdade.

   POR QUE A SESSAO SOME. O refresh token do Supabase e de uso unico e
   roda a cada renovacao: usa-se um, recebe-se outro. Se o aparelho for
   fechado no meio de um rodizio, se a gravacao no navegador falhar, ou
   se uma renovacao levar recusa, a biblioteca joga a sessao inteira
   fora. Dali em diante nao ha o que "reconectar": nao existe mais
   credencial nenhuma no aparelho, e nenhuma quantidade de tentativas
   automaticas traz de volta.

   A TRAVA. O sistema passa a guardar a PROPRIA copia da sessao e a
   atualiza a cada rodizio — entao ela e sempre tao nova quanto a da
   biblioteca. Quando a biblioteca vier vazia, o sistema devolve a copia
   para ela (`setSession`) e a loja volta sozinha. So depois de essa
   tentativa falhar e que a sessao acabou de verdade — e ai a loja ouve
   isso com todas as letras, com o botao de entrar de novo, em vez de
   esperar a manha inteira por uma reconexao impossivel.

   Sobre guardar isso: o token ja mora no localStorage, gravado pela
   propria biblioteca. A copia fica no mesmo lugar, com a mesma
   exposicao — nao ha risco novo. E ela e APAGADA ao sair, senao sair
   nao seria sair.

   `nexor_sessao` JA EXISTE: e o "manter conectado" do login. Usar o
   mesmo nome apagaria um com o outro. A copia tem chave propria.
   ========================================================== */
var _CHAVE_SESSAO='nexor_sessao_nuvem';
function guardarSessao(ses){
  try{
    if(!ses||!ses.access_token||!ses.refresh_token)return;
    localStorage.setItem(_CHAVE_SESSAO,JSON.stringify({
      access_token:ses.access_token, refresh_token:ses.refresh_token,
      expires_at:ses.expires_at||0, em:Date.now()
    }));
  }catch(e){ _quieto(e,'guardarSessao'); }
}
function sessaoGuardada(){
  try{
    var t=localStorage.getItem(_CHAVE_SESSAO);
    if(!t)return null;
    var o=JSON.parse(t);
    return (o&&o.access_token&&o.refresh_token)?o:null;
  }catch(e){ return null; }
}
function apagarSessaoGuardada(){
  try{ localStorage.removeItem(_CHAVE_SESSAO); }
  catch(e){ _quieto(e,'apagarSessaoGuardada'); }
}
/* Devolve a copia para a biblioteca. Usada quando ela vem vazia. */
async function recuperarSessao(cli){
  var c=sessaoGuardada();
  if(!c||!cli||!cli.auth||!cli.auth.setSession)return null;
  try{
    var r=await cli.auth.setSession({access_token:c.access_token,
                                     refresh_token:c.refresh_token});
    var ses=(r&&r.data&&r.data.session)||null;
    if(ses){
      logNuvem('sessão recuperada da cópia guardada neste aparelho');
      guardarSessao(ses);
      return ses;
    }
    /* a copia tambem nao vale mais: insistir com ela so gasta tentativa */
    apagarSessaoGuardada();
    logNuvem('a cópia da sessão também não vale mais — é preciso entrar de novo',true);
  }catch(e){ _quieto(e,'recuperarSessao'); }
  return null;
}
var _cli=null;
function cliente(){
  ...
  try{
    _cli.auth.onAuthStateChange(function(ev,ses){
      if(ses&&ses.access_token){
        ...
        guardarSessao(ses);          /* copia nossa, atualizada a cada rodizio */
      ...
      if(ev==='SIGNED_OUT'){NUVEM.token=null;NUVEM.ligada=false;
        apagarSessaoGuardada();estadoNuvem('offline');}
    });
...
async function conectarNuvem(email,senha){
  ...
  try{limparAvisoSessao();}catch(e){_quieto(e,'conectarNuvem')}
  NUVEM.token=r.data.session?r.data.session.access_token:null;
  guardarSessao(r.data.session);   /* entrou: a copia nasce aqui */
...
async function religarNuvem(){
  ...
  try{
    ...
    var ses=r&&r.data?r.data.session:null;
    /* ==========================================================
       SEM SESSAO NAO E "SEM SERVIDOR"

       Aqui estava so `if(!ses){ estadoNuvem('offline'); return false; }`.
       Sem sessao, o sistema se declarava offline, o relogio de 8 segundos
       chamava esta funcao de novo, e de novo, e de novo — sempre com o
       mesmo vazio. A loja passou a manha de 29/08/2026 vendo "servidor
       nao respondeu — reconectando sozinho" sem que houvesse servidor
       nenhum fora do ar e sem que nenhuma reconexao fosse possivel.

       Duas coisas mudam. Primeiro TENTA de verdade: devolve a copia da
       sessao guardada neste aparelho, que e o caso comum (a biblioteca
       perdeu a dela num rodizio de token). So se isso tambem falhar e
       que a sessao acabou mesmo — e ai a loja ouve a verdade, com o
       botao de entrar de novo, em vez de esperar por nada.
       ========================================================== */
    if(!ses){
      ses=await recuperarSessao(cli);
      if(ses)estadoNuvem('conectando');
    }
    if(!ses){
      /* O aviso e para quem ESTA trabalhando. Na tela de login ele so
         assustaria quem ainda nem entrou — e nao ha sessao para expirar
         antes de existir uma. */
      var _ap=document.getElementById('app');
      var _dentro=_ap&&!_ap.classList.contains('hide');
      if(navigator.onLine&&_dentro){
        NUVEM.sessaoCaiu=true;
        try{avisoSessaoCaiu();}catch(e){_quieto(e,'religarNuvem')}
      }
      estadoNuvem('offline'); return false;
    }
    guardarSessao(ses);
    /* ==========================================================
 ...
 {col:'caixas',      tab:'caixas',
  ...
  filhos:[{lista:'movimentos',tab:'caixa_movimentos',pai:'caixa_id',
    ...
    campos:function(o){return {tipo:o.tipo,valor:n(o.valor),
      ...
      data_hora:dataParaNuvem(o.data)}}}]},
...
/* ==========================================================
   UMA DATA EM FORMATO BRASILEIRO DERRUBA O LOTE INTEIRO

   As colunas de data do banco sao `date` e `timestamptz`. Elas aceitam
   "2026-08-31" e "2026-09-01T02:24:00.000Z"; recusam "31/08/2026" com
   "invalid input syntax", que o PostgREST devolve como erro 400 — e o 400
   derruba o LOTE inteiro, nao so a linha errada.

   Isso importa porque o sistema tem registro gravado por versoes antigas,
   e nem toda tela gravava a data no mesmo formato. Um unico movimento
   antigo com a data no formato do Brasil trava a subida de tudo o que
   estiver no mesmo lote, para sempre, a cada sincronizacao.

   Aqui a data e normalizada antes de sair: o que ja esta certo passa
   intacto; "31/08/2026" e "31/08/2026 23:24" viram ISO; e o que nao for
   data de jeito nenhum vira nulo, em vez de derrubar o envio. Perder o
   carimbo de uma linha antiga e ruim; perder o lote inteiro toda vez, e
   com ele a sangria de hoje, e muito pior.
   ========================================================== */
function dataParaNuvem(v){
  if(v===null||v===undefined||v==='')return null;
  var t=String(v).trim();
  if(!t)return null;
  /* ja esta no formato que o banco entende */
  if(/^\d{4}-\d{2}-\d{2}([T ]|$)/.test(t))return t;
  /* dd/mm/aaaa, com hora ou sem */
  var m=/^(\d{2})\/(\d{2})\/(\d{4})(?:[ T](\d{2}:\d{2}(?::\d{2})?))?$/.exec(t);
  if(m)return m[3]+'-'+m[2]+'-'+m[1]+(m[4]?'T'+(m[4].length===5?m[4]+':00':m[4]):'');
  var d=new Date(t);
...
/* ==========================================================
   SINAL DE VIDA DO APARELHO (Etapa 2 da RDS, 23/09/2026)

   A nuvem sabia o que cada LOJA mandou, mas nao sabia QUAL aparelho
   mandou, nem se algum aparelho parou de sincronizar. Um tablet
   esquecido sem internet guardava vendas por dias e ninguem via — so
   aparecia quando o caixa nao batia.

   Agora, a cada envio ou download que termina bem (e a cada erro de
   envio), o aparelho avisa a nuvem: quem e ele, de que unidade, em que
   versao, se ficou coisa pendente. A API mostra isso em
   /saude-sincronizacao.

   Regras deste aviso, porque ele NUNCA pode atrapalhar a venda:
   - no maximo um por tipo a cada 2 minutos;
   - vai direto, sem repetir e sem tentar reconectar (nao usa `api()`,
     que renova sessao e religa a nuvem quando recebe recusa);
   - qualquer falha e engolida em silencio: o aviso e dispensavel, a
     venda nao.
   ========================================================== */
var _SINAL={ultimo:{},intervalo:120000};
function idDoAparelho(){
  try{
    var id=localStorage.getItem('joia_aparelho_id');
    if(!id){
      id='ap_'+Date.now().toString(36)+Math.random().toString(36).slice(2,10);
      localStorage.setItem('joia_aparelho_id',id);
    }
    return id;
  }catch(e){ return ''; }
}
function sinalDoAparelho(evento,erroTxt){
  try{
    if(!NUVEM.ligada||NUVEM.plataforma||!NUVEM.url||!NUVEM.token)return false;
    var agora=Date.now();
    if(agora-(_SINAL.ultimo[evento]||0)<_SINAL.intervalo)return false;
    var id=idDoAparelho();
    if(!id)return false;
    _SINAL.ultimo[evento]=agora;
    var suc='';try{suc=lojaAtualId()||'';}catch(e){suc='';}
    var p={aparelho_id:id,evento:evento,sucursal_ref:suc,
      usuario:(typeof SESSAO!=='undefined'&&SESSAO&&SESSAO.login)||'',
      versao:(typeof VERSAO!=='undefined'?String(VERSAO):''),
      navegador:String((typeof navigator!=='undefined'&&navigator.userAgent)||'').slice(0,200),
      pendente:!!(NUVEM.sujo||DB._sujo)};
    if(erroTxt)p.erro=String(erroTxt).slice(0,300);
    fetch(NUVEM.url+'/rest/v1/rpc/aparelho_sinal',{method:'POST',keepalive:true,
      headers:{'apikey':NUVEM.chave,'Content-Type':'application/json',
               'Authorization':'Bearer '+NUVEM.token},
      body:JSON.stringify({p:p})}).catch(function(){});
    return true;
  }catch(e){ return false; }
}
...
async function sincronizar(){
  ...
  try{
    ...
    sinalDoAparelho('envio');      /* avisa a nuvem que este aparelho está em dia */
  }catch(e){
    console.error('[JOIA sync]',etapa,e);
    sinalDoAparelho('erro',etapa+': '+((e&&e.message)||'falha'));
